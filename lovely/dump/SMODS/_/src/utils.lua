LOVELY_INTEGRITY = 'ff7922525832d6670064e56a150110ca057a2d44dade17bfceecaf6ed7147d88'
...
function SMODS.calculate_main_scoring(context, scoring_hand)
    for _, card in ipairs(context.cardarea.cards) do
        local in_scoring = context.all_scoring or scoring_hand and SMODS.in_scoring(card, context.scoring_hand)
...
function SMODS.get_card_areas(_type, _context)
    if _type == 'playing_cards' then
        ...
        if next(SMODS.find_card('j_jen_faceless')) and _context == 'main_scoring' then t[#t+1] = {all_scoring = true, cardarea = G.play} end
        return t
    end
    if _type == 'jokers' then
