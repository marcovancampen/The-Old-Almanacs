...
function progress_straddle(add)
  ...
  if Jen.config.straddle.skip_animation then
  ...
  if (progressbar or {})[to_next] then progressbar[to_next]:add_dynatext(nil,
      'Straddle ' .. number_format(G.GAME.straddle + 1)) end
  jl.rd(0.5)
      ...
      jl.a('Straddle ' .. number_format(G.GAME.straddle), G.SETTINGS.GAMESPEED * 2, 1,
        mix_colours(G.C.RED, G.C.UI.TEXT_LIGHT,
          math.min(1 + (Jen.config.straddle.progress_increment / 10),
            G.GAME.straddle / Jen.config.straddle.progress_increment) - (Jen.config.straddle.progress_increment / 10)))
      Q(function()
        for i = 1, MAX do
  ...
  if spd >= 8 or Jen.config.straddle.skip_animation then
    ...
    if orig_straddle ~= G.GAME.straddle then jl.a('Straddle ' .. number_format(G.GAME.straddle), G.SETTINGS.GAMESPEED * 2,
        1,
        mix_colours(G.C.RED, G.C.UI.TEXT_LIGHT,
          math.min(1 + (Jen.config.straddle.progress_increment / 10),
            G.GAME.straddle / Jen.config.straddle.progress_increment) - (Jen.config.straddle.progress_increment / 10))) end
    Q(function()
      play_sound('jen_straddle_increase')
      play_sound('generic1')
      return true
...
function calculate_reroll_cost(final_free)
  ...
  if Jen.config.punish_reroll_abuse then
    if numrolls > 1 then
      G.GAME.current_round.reroll_cost = math.min(math.ceil(G.GAME.current_round.reroll_cost * (1.13 ^ (numrolls - 1))),
        1e100)
    end
  end
end
