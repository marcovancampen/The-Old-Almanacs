		...
		if G and G.E_MANAGER then
			...
			if #self.memory_history >= 5 then
				...
				if self.memory_trend > 1000 and memory > 150000 then -- Increasing by >1MB per check and already high
					print("[JEN DEBUG] ⚠️ MEMORY LEAK DETECTED - Memory increasing by", math.floor(self.memory_trend),
						"KB per check")
...
function jl.scj(context)
	return context.cardarea and context.cardarea == G.play and not context.before and not context.after and
...
function QR(fc, count, de, t, tr, bl, ba)
	...
	Q(function()
		if count <= 0 then fc() else Q(fc, count - 1, de, t, tr, bl, ba) end
		return true
	end)
end
...
function jl.rnd(seed, excluded_flags, pool, ignore_pooling, attempts)
	...
	while true do
		...
		selection = G.P_CENTERS
			[pseudorandom_element(pool or G.P_CENTER_POOLS.Consumeables, pseudoseed(seed or 'jlrnd')).key]
...
function jl.card(key)
	if G.P_CENTERS[key] then
		return create_card(G.P_CENTERS[key].set, (G.P_CENTERS[key].set == 'Joker' and G.jokers or G.consumeables), nil,
			nil, nil, nil, key, 'jlspecificcard')
	end
	return nil
end
...
function jl.voucher(key)
	...
	if G.STATE == G.STATES.HAND_PLAYED then
		if not G.redeemed_vouchers_during_hand then
			G.redeemed_vouchers_during_hand = CardArea(G.play.T.x, G.play.T.y, G.play.T.w, G.play.T.h,
	...
	local card = Card(area.T.x + area.T.w / 2 - G.CARD_W / 2, area.T.y + area.T.h / 2 - G.CARD_H / 2, G.CARD_W, G.CARD_H,
		G.P_CARDS.empty, G.P_CENTERS[voucher_key], { bypass_discovery_center = true, bypass_discovery_ui = true })
...
function jl.setup_pointer_blacklist()
	...
	-- Blacklist Godsmarble
	Cryptid.pointerblistify("j_jen_godsmarble")
	print("[JEN DEBUG] Blacklisted Godsmarble from pointer")

	-- Blacklist Goob and his hands
	Cryptid.pointerblistify("j_jen_goob")
	Cryptid.pointerblistify("j_jen_goob_lefthand")
	Cryptid.pointerblistify("j_jen_goob_righthand")
	print("[JEN DEBUG] Blacklisted Goob and hands from pointer")

	-- Blacklist Omega cards individually
	local omega_cards = {
		"c_jen_soul_omega",
		"c_jen_ectoplasm_omega",
		"c_jen_familiar_omega",
		"c_jen_grim_omega",
		"c_jen_hex_omega",
		"c_jen_immolate_omega",
		"c_jen_incantation_omega",
		"c_jen_ouija_omega",
		"c_jen_sigil_omega",
		"c_jen_wraith_omega",
		"c_jen_fool_omega",
		"c_jen_magician_omega",
		"c_jen_empress_omega",
		"c_jen_hierophant_omega",
		"c_jen_lovers_omega",
		"c_jen_chariot_omega",
		"c_jen_justice_omega",
		"c_jen_devil_omega",
		"c_jen_tower_omega",
		"c_jen_high_priestess_omega",
		"c_jen_emperor_omega",
		"c_jen_hermit_omega",
		"c_jen_wheel_of_fortune_omega",
		"c_jen_world_omega",
		"c_jen_sun_omega",
		"c_jen_star_omega",
		"c_jen_moon_omega",
	...
	for _, key in ipairs(omega_cards) do
		Cryptid.pointerblistify(key)
	end
	print("[JEN DEBUG] Blacklisted individual Omega cards from pointer")

	-- Blacklist Icon Pack
	Cryptid.pointerblistify("p_jen_iconpack")
	print("[JEN DEBUG] Blacklisted Icon Pack from pointer")
